  ...
  Q.component("platformerControls", {
    ...
    step: function(dt) {
      ...
      if(p.ignoreControls === undefined || !p.ignoreControls) {
        ...
        // Controls the slide time.
        if(p.slideCounter > 0 && Q.inputs['down'] && p.slidePossible) {
          p.slideCounter = p.slideCounter - 1;
          p.sliding = true;
        }
        else if(p.slideCounter <= 0 || (p.sliding && !Q.inputs['down'])){
          p.sliding = false;
          p.slidePossible = false;
          p.slideCounter = p.slideCounterInit;
        }

        // Controls the slide countdown.
        if(!p.slidePossible && p.slideCountdown > 0) {
          p.slideCountdown = p.slideCountdown - 1;
        } else if(p.slideCountdown <= 0) {
          p.slidePossible = true;
          p.slideCountdown = p.slideCountdownInit;
        }
        ...
        if(Q.inputs['left']) {
          ...
          if(collision && p.landed > 0) {
            ...
            if(Q.inputs['down'] && p.slidePossible) {
              p.vx -= p.slideSpeed;
              p.y += 107;
            }
          } else {
        ...
        if(p.landed > 0 && (Q.inputs['up']/* || Q.inputs['action']*/) && !p.jumping) {
          ...
          p.landed = -dt;
          p.jumping = true;
        } else if(p.landed < 0 && (Q.inputs['up']/* || Q.inputs['action']*/) && !p.jumping  && !p.doubleJump) {
          p.vy = p.jumpSpeed;
          p.landed = -dt;
          p.jumping = true;
          p.doubleJump = true;
        ...
        if(p.landed > 0 && p.doubleJump)
          p.doubleJump = false;

        if(Q.inputs['fire'] && p.landed < 0 && p.vy > 0) {
            p.vy += p.flySpeed;
            p.flying = true;
        }
