 ...
 Q.Sprite.extend("TileLayer",{
  ...
  Q.component('2d',{
    ...
    collision: function(col,last) {
      ...
      // Bottom-top remote collision.
      if(col.normalY < -0.3) {
        if(!p.skipCollide && p.vy > 0) { p.vy = 0; }
        col.impact = impactY;
        entity.trigger("bump.bottom-remote",col);
      }
      if(col.normalY > 0.3) {
        if(!p.skipCollide && p.vy < 0) { p.vy = 0; }
        col.impact = impactY;

        entity.trigger("bump.top-remote",col);
      }

      // Right-left remote collision.
      if(col.normalX < -0.3) {
        if(!p.skipCollide && p.vx > 0) { p.vx = 0;  }
        col.impact = impactX;

        entity.trigger("bump.right-remote",col);
      }
      if(col.normalX > 0.3) {
        if(!p.skipCollide && p.vx < 0) { p.vx = 0; }
        col.impact = impactX;

        entity.trigger("bump.left-remote",col);
      }
    },
